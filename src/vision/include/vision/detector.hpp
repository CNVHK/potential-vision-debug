...
namespace auto_aim {
    class Detector {
    public:
        Detector(const std::string & config_path, const Color &color, bool debug = true);
        std::vector<Armor> detect_armors(const cv::Mat &input_img);
    ...
    private:
        ...
        std::vector<Armor> match_armors(const std::vector<Lightbar> &lightbars);
