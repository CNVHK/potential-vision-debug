    ...
    capture_thread_ = std::thread{[this]() -> void {
      ...
      while (rclcpp::ok()) {
        ...
        if (MV_OK == nRet) {
          ...
          //std::cout << image_msg_.height<< " " << image_msg_.width << " " << image_msg_.step << std::endl;
          image_msg_.data.resize(image_msg_.width * image_msg_.height * 3);//3
...
private:
  void declareParameters()
  {
    ...
    int ret_exp_auto = MV_CC_SetEnumValue(camera_handle_, "ExposureAuto", 0);
    int ret_gain_auto = MV_CC_SetEnumValue(camera_handle_, "GainAuto", 0);
    if (ret_exp_auto != MV_OK || ret_gain_auto != MV_OK) {
        RCLCPP_WARN(this->get_logger(), "Failed to turn off Auto Exposure or Auto Gain!");
    } else {
        RCLCPP_INFO(this->get_logger(), "Auto Exposure and Auto Gain are successfully disabled.");
    }
    ...
    double exposure_time = this->declare_parameter("exposure_time", 2000, param_desc); //5000us
